...
TaskHandle_t hOledDisp_Task = NULL;
QueueHandle_t hOledDisp_Queue = NULL;
...
void OLED_Display_Task(void *pvParameters)
{
    ...
    OLED_P8x16Str(0,0,"Vol:", F8X16);
    OLED_P8x16Str(0,16,"Cur:", F8X16);
    OLED_P8x16Str(0,32,"Pow:", F8X16);

    OLED_P8x16Str(88,0,"V", F8X16);
    OLED_P8x16Str(88,16,"mA", F8X16);
    OLED_P8x16Str(88,32,"W", F8X16);
    
    while(pdTRUE)
    {
        xRet = FR_OS_QueueReceive(hOledDisp_Queue, &stOledDisp, portMAX_DELAY);
        if(xRet == pdTRUE)
        {
            //taskENTER_CRITICAL();
            sprintf(pu8Buff,"%6.2f",stOledDisp.f32data);
            OLED_P8x16Str(stOledDisp.u8x,stOledDisp.u8y,pu8Buff, F8X16);
            //taskEXIT_CRITICAL();
        }
        
        /*snprintf(pu8Buff,10,"%f",stInaInfo.f32Vbus);
        OLED_P8x16Str(0,0,pu8Buff, F8X16);
        //OLED_P8x16Str(0,0,"12345", F8X16);
        FR_OS_DelayMs(200);*/

        //snprintf(pu8Buff,10,"%f",stInaInfo.f32Cur); 
        //OLED_P8x16Str(0,20,pu8Buff, F8X16);
        //FR_OS_DelayMs(200);

        /*OLED_P8x16Str(0,u8RowCnt,"04", F8X16);u8RowCnt+=16;
        FR_OS_DelayMs(200);
        OLED_P8x16Str(0,u8RowCnt,"05", F8X16);u8RowCnt+=16;
        FR_OS_DelayMs(200);*/
        u8RowCnt = 0;
    }
}
